...
/// The PINNED unpacker verify [`CircuitConfig`] — the single `RECURSION_CONFIG` rebuilt (`to_derived`)
/// at the node/leaf blowup it was captured with. Asserts `n == CAPTURED_N` (the leaf count is pinned).
fn pinned_unpacker_config(n: usize) -> circuit_verifier::verify::CircuitConfig {
    assert_eq!(
        n,
        recursion_consts::CAPTURED_N,
        "unpacker config requested for n={n} but the pinned config has N={}",
        recursion_consts::CAPTURED_N
    );
    recursion_consts::RECURSION_CONFIG.unpacker_config()
}
...
fn verify_gate_air_root_leaves(
    ...
    let verify_config = pinned_unpacker_config(n);
...
fn prove_folded(
    ...
    let shots_per_shard_raw: usize = parse_env::<usize>("RECURSION_SHARD_SHOTS")
        .filter(|&n| n > 0)
        .unwrap_or_else(|| recursion_consts::shots_per_shard(k));
    ...
    // The pinned recursion consts (leaf/level1/fold roots + unpacker config) were captured for one
    // `(k, shots, N)`; a mismatch would prove/verify against the wrong pinned shape, so fail loud and
    // point at the regen script instead. Regenerate `src/recursion_consts.rs` for this `k` via
    // `scripts/regen_recursion_consts.sh <k>`.
    assert_eq!(
        (k, shots_per_shard_raw, n_shards),
        (
            recursion_consts::CAPTURED_K,
            recursion_consts::CAPTURED_SHOTS,
            recursion_consts::CAPTURED_N
        ),
        "run (k={k}, shots={shots_per_shard_raw}, N={n_shards}) != pinned recursion_consts \
         (k={}, shots={}, N={}); regenerate src/recursion_consts.rs for this k with \
         scripts/regen_recursion_consts.sh {k}",
        recursion_consts::CAPTURED_K,
        recursion_consts::CAPTURED_SHOTS,
        recursion_consts::CAPTURED_N,
    ...
    let recursion_pre = {
        ...
        // from there); gate-air holds only the pinned const `RECURSION_CONFIG`.
        let pre = recursive_aggregate::precomputes::build_recursion_precompute(
            ...
            &recursion_consts::RECURSION_CONFIG,
            false,
        );
        eprintln!(
    ...
    let (base_nodes, out, rv) = {
        ...
        let unpacker_config = pinned_unpacker_config(leaves.len());
        let t = Instant::now();
        ...
        let tv = Instant::now();
        verify_gate_air_root_leaves(&rv, n_leaves)
