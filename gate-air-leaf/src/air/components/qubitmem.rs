//! Qubit-memory supply component. Owns the qubit-memory relation id and the `QubitMemEval`
//! qubitmem table. Relocated verbatim from `air.rs` (byte-identical constraint code).
...
/// Qubit-memory table (supply side). Per (shot, addr) — `shot`/`addr` preprocessed,
...
impl FrameworkEval for QubitMemEval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        // Real-row enabler (1 on real (shot,addr), 0 on padding). Gates the qubitmem emission so
        // padding rows (non-power-of-two n_shots*N_QUBITS) inject no unmatched LogUp terms.
        let qmem_enabler = eval.get_preprocessed_column(pp_id("gate_bnd_enabler"));
        ...
        // Phase-3 x/y binding (re-keyed qubitmem). The init anchor is NO LONGER consumed here:
        // main's first Use +1/(shot,addr,0,x) is left DANGLING as a PUBLIC term. The qubitmem now
        ...
        // emitted by the qubitmem — it lives only on main's dangling init term.)
        // (B) internal final Use[+qmem_enabler]: +qmem_enabler / (shot, addr, ts_last, y).
        eval.add_to_relation(RelationEntry::new(
            &self.elements,
            E::EF::from(qmem_enabler.clone()),
            &[tag.clone(), shot.clone(), addr.clone(), ts_last, y.clone()],
        ));
        // (D) public final Yield[-qmem_enabler]: -qmem_enabler / (shot, addr, TS_FINAL, y).
        eval.add_to_relation(RelationEntry::new(
            &self.elements,
            -E::EF::from(qmem_enabler),
